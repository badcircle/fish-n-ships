...
@export_range(20,4000, 1) var Terrain_Size := 1000
@export_range(1, 100, 1) var resolution := 100
@export var coastline := 5
	...
	for z in resolution+1:
		for x in resolution+1:
			...
			if x <= coastline:
				if grid_coord.x == -1:
					adjust = true
			if z >= resolution - coastline:
				if grid_coord.y == 2:
					adjust = true
			if x >= resolution - coastline:
				if grid_coord.x == 2:
					adjust = true
			if adjust:
				if vertex.y > 1:
